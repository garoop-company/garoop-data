Create Codex pet row `running-right` for `garuchan-spy`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: ピンクの大きな耳と大きな瞳を持つかわいいガルちゃん。お腹のポーチに赤ちゃんがいる。スパイ映画風の身のこなしだが、衣装や武器は追加せず、既存の赤ちゃんポーチの特徴を保つ。. Preserve silhouette, face, palette, material, proportions, markings, and props.
