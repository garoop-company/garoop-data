Create Codex pet row `running-right` for `rakopi`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: pink sea otter mascot holding a small shell attached to body, cream muzzle and belly, rounded tail. Reference style lock: soft pastel pink base, deep rose thick rounded outlines, huge glossy round eyes with white star highlights, rosy cheeks, cream-white muzzle/chest/belly accents, plush baby proportions, cute front-facing mascot, clean whole-body sprite, no black background, no shadow.. Japanese display concept: らこぴ: 貝をだっこするラッコ。お気に入りを大切にしまう、ぷかぷか気分の癒し係。. Preserve silhouette, face, palette, material, proportions, markings, and props.
